...
export function ClientForm({ onSaved }: { onSaved: () => void }) {
  const form = useForm<z.infer<typeof clientSchema>>({ resolver: zodResolver(clientSchema), defaultValues: { businessName: "", contactName: "", phone: "", email: "", service: "", city: "", state: "", plan: "Plan base", setupFee: 400, monthlyFee: 200, status: "ACTIVO", startDate: new Date().toISOString().split("T")[0] } });
  ...
  return (
    <form onSubmit={onSubmit} className="grid md:grid-cols-2 gap-3">
      ...
      <Select defaultValue="ACTIVO" onValueChange={(v) => form.setValue("status", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ACTIVO">Activo</SelectItem><SelectItem value="PAUSADO">Pausado</SelectItem><SelectItem value="CANCELADO">Cancelado</SelectItem></SelectContent></Select>

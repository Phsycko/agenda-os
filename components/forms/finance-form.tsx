...
export function FinanceForm({ onSaved }: { onSaved: () => void }) {
  const form = useForm<z.infer<typeof financeSchema>>({ resolver: zodResolver(financeSchema), defaultValues: { type: "INCOME", category: "Setup", amount: 0, concept: "", date: new Date().toISOString().split("T")[0] } });
  return <form className="grid gap-3" onSubmit={form.handleSubmit(async (v)=>{ await fetch('/api/finance',{method:'POST', body: JSON.stringify(v)}); onSaved(); form.reset();})}>
    <div className="grid grid-cols-2 gap-3">
      <Select defaultValue="INCOME" onValueChange={(v)=>form.setValue("type", v as "INCOME"|"EXPENSE")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="INCOME">Ingreso</SelectItem><SelectItem value="EXPENSE">Egreso</SelectItem></SelectContent></Select>
